...
class StableDiffusionAdaptiveTokenPipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin, FromSingleFileMixin):
    ...
    def __call__(
        ...
        # ===== DELAYED INJECTION (race) =====
        ori_prompt_embeds = None
        if injection_step > 0 and base_prompt is not None:
            ori_prompt_embeds, _ = self.encode_prompt(
                base_prompt,
                device,
                num_images_per_prompt,
                do_classifier_free_guidance,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # DELAYED INJECTION: before injection_step use base (no-token)
                # embeds to form a coherent scene; after, use inclusive (token) embeds.
                if ori_prompt_embeds is not None and i < injection_step:
                    active_prompt_embeds = ori_prompt_embeds
                else:
                    active_prompt_embeds = prompt_embeds

                if do_classifier_free_guidance:
                    encoder_hidden_states = torch.cat(
                    [negative_prompt_embeds, active_prompt_embeds]
                    )
                else:
