...
# =========================================================
# train_race.sh
# SPATM RACE Bias Training (JarvisLabs)
#
# Usage:
#   chmod +x train_race.sh
#   ./train_race.sh                      # uses defaults below
#   ./train_race.sh /path/to/race_dataset.txt   # override dataset CSV
#
# Env overrides:
...
# checkpoint with run_race.sh instead.
# =========================================================

set -euo pipefail

SEED=666
NOTE="race-spatm-v2-anchor1"
...
DATA_DIR="${1:-${DATASET_ROOT}/race_dataset/race_dataset.txt}"

if [ ! -f "$DATA_DIR" ]; then
  echo "ERROR: dataset CSV not found: $DATA_DIR"
  echo "  Build it first, e.g.:"
  echo "    DATASET_ROOT=$DATASET_ROOT BUILD_CSV=1 ./generate_data.sh race train"
  exit 1
fi

echo "Training SPATM for RACE bias"
...
echo ""
echo "========================================="
echo "RACE training complete."
if [ -n "$CKPT_DIR" ]; then
  echo "Checkpoint: $CKPT_DIR"
  echo "Evaluate with:"
  echo "  ./run_race.sh \"$CKPT_DIR\""
else
